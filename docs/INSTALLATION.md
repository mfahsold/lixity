...
## CLI: current development version
...
The current checkout is **1.15.0.dev0**, not a published 1.15.0 release.
`main` includes the new UI and shared pipeline and can change. For reproducible
automation, replace `main` with a reviewed full commit hash.
...
If you prefer the older published release, use this **instead**:

```sh
uv tool install --python 3.12 "git+https://github.com/mfahsold/lixity.git@v1.14.0"
```

New development features are not included in that release. Review
...
`pipx install "git+https://github.com/mfahsold/lixity.git@main"`, followed by
...
```sh
python3 -m venv .venv
.venv/bin/python -m pip install "git+https://github.com/mfahsold/lixity.git@main"
...
```powershell
py -3.12 -m venv .venv
.\.venv\Scripts\python.exe -m pip install "git+https://github.com/mfahsold/lixity.git@main"

...
def render_dashboard(
    ...
    if controls:
        ...
        parts.append('<p class="ctl-note" id="research-hint">Lokale Evidenzen verwalten, indizieren und mit dem Manuskript abgleichen.</p>')
        ...
        parts.append('      <div class="file-dropzone dropzone-compact" id="open-dropzone" tabindex="0" role="button" aria-label="Manuskriptdatei auswählen oder ablegen">')
        parts.append('        <input type="file" id="open-file-input" accept=".md,.markdown,.txt" style="display:none;" />')
        parts.append('        <div class="dropzone-text">')
        parts.append(f'          <strong>{"Manuskript (.md) direkt öffnen" if is_de else "Open manuscript (.md) directly"}</strong>')
        parts.append(f'          <span>{"Datei hierher ziehen oder klicken zum Auswählen" if is_de else "Drop file here or click to browse"}</span>')
        parts.append('        </div>')
        parts.append('      </div>')
        parts.append('      <div class="form-group" style="margin-top:.4rem;">')
        parts.append(f'        <label for="open-proj-path" class="form-label">{"Oder Pfad eingeben:" if is_de else "Or enter folder / file path:"}</label>')
        parts.append(f'        <input class="ctl" id="open-proj-path" placeholder="{"/pfad/zu/deinem/projekt oder manuskript.md" if is_de else "/path/to/project or manuscript.md"}" style="width:100%;"/>')

...
def render_dashboard(
    ...
    if not chapters:
        ...
        if is_de:
            parts.append('      <div class="og-header">')
            parts.append('        <h3>Schnelleinstieg in Lixity</h3>')
            parts.append('        <p>Lixity arbeitet vollständig lokal und dateibasiert ohne Cloud-Zwang. Ein Projekt besteht aus drei Kernbausteinen:</p>')
            parts.append('      </div>')
            parts.append('      <div class="og-steps">')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">1</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Manuskript (manuscript.md)</strong>')
            parts.append('            <p>Ihr Fließtext in Markdown mit Kapitelüberschriften (<code># Kapitel 1</code>).</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">2</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Projektprofil (lixity.json)</strong>')
            parts.append('            <p>Titel, Zielsprache und individuelle Grenzwerte für die Stil-Diagnostik.</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">3</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Recherchespeicher (research/)</strong>')
            parts.append('            <p>Historische Quellen, Quellzitate, Thesen und dokumentierte Abweichungen.</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('      </div>')
        else:
            parts.append('      <div class="og-header">')
            parts.append('        <h3>Quickstart & Project Structure</h3>')
            parts.append('        <p>Lixity runs entirely local and file-based with no cloud requirement. A project consists of three core components:</p>')
            parts.append('      </div>')
            parts.append('      <div class="og-steps">')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">1</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Manuscript (manuscript.md)</strong>')
            parts.append('            <p>Your prose in Markdown with structured chapter headings (<code># Chapter 1</code>).</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">2</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Project Config (lixity.json)</strong>')
            parts.append('            <p>Title, target language, and customized thresholds for stylistic diagnostics.</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('        <div class="og-step">')
            parts.append('          <div class="og-step-num">3</div>')
            parts.append('          <div class="og-step-body">')
            parts.append('            <strong>Research Archive (research/)</strong>')
            parts.append('            <p>Historical sources, passage citations, claims, and documented plot deviations.</p>')
            parts.append('          </div>')
            parts.append('        </div>')
            parts.append('      </div>')
        parts.append('    </div>')
    ...
    if controls:
        ...
        parts.append('<p class="ctl-note" id="research-hint" style="margin:0;">Lokale Evidenzen verwalten, indizieren und mit dem Manuskript abgleichen.</p>')
        ...
        parts.append('<button type="button" class="ctl active" data-rtab="sources" role="tab" aria-selected="true">Quellen</button>')
        parts.append('<button type="button" class="ctl" data-rtab="search" role="tab" aria-selected="false">Suche</button>')
        parts.append('<button type="button" class="ctl" data-rtab="dossiers" role="tab" aria-selected="false">Dossiers</button>')
        parts.append('<button type="button" class="ctl" data-rtab="claims" role="tab" aria-selected="false">Thesen &amp; Belege</button>')
        parts.append('<button type="button" class="ctl" data-rtab="decisions" role="tab" aria-selected="false">Entscheidungen</button>')
        parts.append('<button type="button" class="ctl" data-rtab="grounding" role="tab" aria-selected="false">Abgleich</button>')
        parts.append('</div>')

        parts.append('<div id="research-init-box" class="ctl-group" style="display:none;">')
        parts.append('<span class="ctl-label">Recherchespeicher nicht initialisiert</span>')
        parts.append('<p class="ctl-note">Für dieses Projekt existiert noch kein lokaler Recherchespeicher (<code>research/</code>).</p>')
        parts.append('<div class="row">')
        parts.append('<input class="ctl" id="r-init-title" placeholder="Projekttitel" style="min-width:200px;"/>')
        parts.append('<button class="ctl primary" id="r-init-btn">Recherchespeicher anlegen</button>')
        parts.append('</div>')
        parts.append('</div>')

        parts.append('<div class="research-tab-pane" id="rtab-sources">')
        parts.append('<div class="ctl-group">')
        parts.append('<span class="ctl-label">Quelle erfassen (Ingest)</span>')
        ...
        parts.append('<input class="ctl" id="r-ingest-title" placeholder="Titel / Quelle" style="min-width:180px;"/>')
        parts.append('<input class="ctl" id="r-ingest-tags" placeholder="Tags (z. B. archiv, 1924)" style="min-width:180px;"/>')
        parts.append('</div>')
        parts.append('<div class="row" style="margin-bottom:.5rem;">')
        parts.append('<textarea class="ctl" id="r-ingest-text" placeholder="Oder Quelltext direkt hier eingeben..." rows="3" style="width:100%;font-family:inherit;"></textarea>')
        parts.append('</div>')
        parts.append('<div class="row" style="align-items:center;">')
        parts.append('<label style="display:flex;align-items:center;gap:.3rem;font-size:.85rem;cursor:pointer;"><input type="checkbox" id="r-ingest-retention"/> <span>Lokale Speicherung bestätigen (<code>--allow-retention</code>)</span></label>')
        parts.append('<button class="ctl primary" id="r-ingest-btn">Quelle speichern & indizieren</button>')
        parts.append('</div>')
        parts.append('</div>')
        ...
        parts.append('<div class="ctl-group">')
        parts.append('<span class="ctl-label">Volltextsuche in Quellen (FTS5 BM25)</span>')
        parts.append('<div class="row">')
        parts.append('<input class="ctl" id="r-search-query" placeholder="Suchbegriff oder Phrase eingeben..." style="min-width:240px;flex:1;"/>')
        parts.append('<button class="ctl primary" id="r-search-btn">Suchen</button>')
        parts.append('</div>')
        parts.append('</div>')
        parts.append('<div id="research-search-results"></div>')
        parts.append('</div>')

        parts.append('<div class="research-tab-pane" id="rtab-dossiers" style="display:none;">')
        parts.append('<div class="ctl-group">')
        parts.append('<span class="ctl-label">Neues Dossier erstellen</span>')
        parts.append('<div class="row" style="margin-bottom:.5rem;">')
        parts.append('<input class="ctl" id="r-dos-title" placeholder="Dossiertitel" style="min-width:220px;"/>')
        parts.append('<input class="ctl" id="r-dos-tags" placeholder="Tags (kommagetrennt)" style="min-width:180px;"/>')
        parts.append('<input class="ctl" id="r-dos-eids" placeholder="Evidenz-Passagen IDs (optional, kommagetrennt)" style="flex:1;min-width:200px;"/>')
        parts.append('</div>')
        parts.append('<div class="row" style="margin-bottom:.5rem;">')
        parts.append('<textarea class="ctl" id="r-dos-body" placeholder="Dossier-Notizen, Thesen oder Zusammenfassung..." rows="4" style="width:100%;font-family:inherit;"></textarea>')
        parts.append('</div>')
        parts.append('<div class="row">')
        parts.append('<button class="ctl primary" id="r-dos-create-btn">Dossier speichern</button>')
        parts.append('</div>')
        parts.append('</div>')
        ...
        parts.append('  <div class="ctl-group">')
        parts.append('    <span class="ctl-label">Neue These / historische Aussage anlegen</span>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <input class="ctl" id="r-claim-title" placeholder="Kurztitel der These" style="min-width:220px;flex:1;"/>')
        parts.append('      <select class="ctl" id="r-claim-confidence" style="min-width:140px;">')
        parts.append('        <option value="hypothetical">Hypothetisch</option>')
        parts.append('        <option value="evidenced">Belegt</option>')
        parts.append('        <option value="disputed">Umstritten</option>')
        parts.append('      </select>')
        parts.append('      <input class="ctl" id="r-claim-tags" placeholder="Tags (kommagetrennt)" style="min-width:160px;"/>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <textarea class="ctl" id="r-claim-statement" placeholder="Vollständige historische Aussage oder Hypothese..." rows="3" style="width:100%;font-family:inherit;"></textarea>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <input class="ctl" id="r-claim-time" placeholder="Zeitraum (z. B. Herbst 1912)" style="flex:1;min-width:150px;"/>')
        parts.append('      <input class="ctl" id="r-claim-place" placeholder="Ort (z. B. Zürich)" style="flex:1;min-width:150px;"/>')
        parts.append('      <input class="ctl" id="r-claim-actors" placeholder="Schlüsselakteure (kommagetrennt)" style="flex:1.5;min-width:180px;"/>')
        parts.append('    </div>')
        parts.append('    <div class="row">')
        parts.append('      <button class="ctl primary" id="r-claim-create-btn">These speichern</button>')
        parts.append('    </div>')
        parts.append('  </div>')
        parts.append('  <div class="ctl-group" style="margin-top:.8rem;">')
        parts.append('    <span class="ctl-label">Passagenbeleg mit These verknüpfen</span>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <select class="ctl" id="r-link-claim-select" style="min-width:220px;flex:1;"><option value="">These auswählen...</option></select>')
        parts.append('      <input class="ctl" id="r-link-passage-id" placeholder="Passage-UUID (z.B. aus Suche)" style="min-width:220px;flex:1;font-family:monospace;"/>')
        parts.append('      <select class="ctl" id="r-link-relation" style="min-width:140px;">')
        parts.append('        <option value="supports">Stützt (supports)</option>')
        parts.append('        <option value="contradicts">Widerspricht (contradicts)</option>')
        parts.append('        <option value="qualifies">Schränkt ein (qualifies)</option>')
        parts.append('        <option value="contextualizes">Kontextualisiert</option>')
        parts.append('      </select>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <input class="ctl" id="r-link-rationale" placeholder="Begründung für die Verknüpfung (optional)" style="flex:1;"/>')
        parts.append('      <button class="ctl primary" id="r-link-evidence-btn">Beleg verknüpfen</button>')
        parts.append('    </div>')
        parts.append('  </div>')
        ...
        parts.append('  <div class="ctl-group">')
        parts.append('    <span class="ctl-label">Künstlerische Entscheidung / historische Abweichung festhalten</span>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <input class="ctl" id="r-decision-title" placeholder="Titel der Entscheidung (z. B. Datumsverschiebung)" style="min-width:220px;flex:1;"/>')
        parts.append('      <select class="ctl" id="r-decision-claim-select" style="min-width:220px;flex:1;"><option value="">Keine These verknüpft</option></select>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <textarea class="ctl" id="r-decision-rationale" placeholder="Künstlerische, historische oder dramaturgische Begründung..." rows="3" style="width:100%;font-family:inherit;"></textarea>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append('      <input class="ctl" id="r-decision-plot" placeholder="Auswirkung auf Handlung oder Figuren (Plot Impact)..." style="flex:1;min-width:250px;"/>')
        parts.append('    </div>')
        ...
        parts.append('        <span>Bewusste Abweichung von historischer Evidenz (dichterische Freiheit)</span>')
        parts.append('      </label>')
        parts.append('      <button class="ctl primary" id="r-decision-create-btn">Entscheidung festhalten</button>')
        parts.append('    </div>')
        parts.append('  </div>')
        ...
        parts.append('<div class="ctl-group">')
        parts.append('<span class="ctl-label">Quelle gegen geladenes Manuskript abgleichen</span>')
        parts.append('<div class="row">')
        parts.append('<select class="ctl" id="r-ground-source-select" style="min-width:220px;"><option value="">Quelle auswählen...</option></select>')
        parts.append('<button class="ctl primary" id="r-ground-btn">Abgleich ausführen</button>')
        parts.append('</div>')
        parts.append('</div>')
        parts.append('<div id="research-grounding-results"></div>')
        parts.append('</div>')
        ...
        is_de = language_key == "de"
        # Modal: New Project (with Import & From-Scratch Tabs)
        parts.append('<dialog class="lixity-modal" id="modal-project-create" closedby="any" aria-labelledby="modal-create-title">')
        parts.append('  <div class="modal-card">')
        parts.append('    <div class="modal-header">')
        parts.append(f'      <h3 id="modal-create-title">{L("create_project")}</h3>')
        parts.append('      <button type="button" class="modal-close" data-close-modal aria-label="Schließen">✕</button>')
        parts.append('    </div>')
        parts.append('    <div class="modal-tabs" role="tablist">')
        parts.append(f'      <button type="button" class="modal-tab-btn active" role="tab" id="tab-btn-import" aria-selected="true" data-tab-target="tab-pane-import">{"Manuskript importieren" if is_de else "Import Manuscript"}</button>')
        parts.append(f'      <button type="button" class="modal-tab-btn" role="tab" id="tab-btn-scratch" aria-selected="false" data-tab-target="tab-pane-scratch">{"Neues Manuskript beginnen" if is_de else "Start New Manuscript"}</button>')
        parts.append('    </div>')

        # Tab Pane 1: Import existing manuscript
        parts.append('    <div class="modal-tab-pane active" id="tab-pane-import">')
        parts.append('      <form id="form-project-import" class="modal-body" method="dialog">')
        parts.append('        <div class="file-dropzone" id="import-dropzone" tabindex="0" role="button" autofocus aria-label="Manuskriptdatei auswählen oder ablegen">')
        parts.append('          <input type="file" id="import-file-input" accept=".md,.markdown,.txt" style="display:none;" />')
        parts.append('          <div class="dropzone-icon">')
        parts.append('            <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="12" y1="18" x2="12" y2="12"></line><line x1="9" y1="15" x2="12" y2="12"></line><line x1="15" y1="15" x2="12" y2="12"></line></svg>')
        parts.append('          </div>')
        parts.append('          <div class="dropzone-text">')
        parts.append(f'            <strong>{"Markdown-Datei (.md, .txt) hierher ziehen" if is_de else "Drop Markdown file (.md, .txt) here"}</strong>')
        parts.append(f'            <span>{"oder klicken, um eine Datei vom Computer auszuwählen" if is_de else "or click to choose a file from your computer"}</span>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="file-preview-card" id="import-preview-box" style="display:none;">')
        parts.append('          <div class="fpc-header">')
        parts.append(f'            <span class="fpc-badge">{"Manuskript erkannt" if is_de else "Manuscript recognized"}</span>')
        parts.append('            <span class="fpc-filename" id="import-fpc-filename">manuscript.md</span>')
        parts.append('          </div>')
        parts.append('          <div class="fpc-meta">')
        parts.append(f'            <div class="fpc-stat"><span class="fpc-label">{"Umfang:" if is_de else "Scope:"}</span> <strong id="import-fpc-stats">0 Wörter · 0 Kapitel</strong></div>')
        parts.append(f'            <div class="fpc-stat"><span class="fpc-label">{"Sprachmuster:" if is_de else "Language:"}</span> <span id="import-fpc-lang">Deutsch</span></div>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="project-structure-hint">')
        if is_de:
            parts.append('          <span><strong>Projektaufbau:</strong> Lixity legt Ihr Manuskript (<code>manuscript.md</code>) und das Projektprofil (<code>lixity.json</code>) strukturiert in einem lokalen Ordner ab.</span>')
        else:
            parts.append('          <span><strong>Project Layout:</strong> Lixity saves your manuscript (<code>manuscript.md</code>) and config (<code>lixity.json</code>) structured in a local folder.</span>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <label for="import-proj-title" class="form-label">{L("title")}</label>')
        parts.append(f'          <input class="ctl" id="import-proj-title" required placeholder="{"Wird automatisch aus der ersten Überschrift gelesen" if is_de else "Extracted from document heading"}" style="width:100%;"/>')
        parts.append('        </div>')
        ...
        parts.append(f'            <label for="import-proj-path" class="form-label">{"Projektordner (optional)" if is_de else "Project folder (optional)"}</label>')
        parts.append(f'            <input class="ctl" id="import-proj-path" placeholder="{"Standard: Automatisch aus Titel" if is_de else "Default: Generated from title"}" style="width:100%;"/>')
        parts.append(f'            <span class="ctl-note" style="display:block;margin-top:.25rem;font-size:.72rem;">{"Ohne Eingabe wird automatisch ein neuer Ordner aus dem Titel erzeugt." if is_de else "If omitted, a new directory based on the title is generated automatically."}</span>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <label class="form-checkbox"><input type="checkbox" id="import-proj-research"/> <span>{"Recherche- und Dossierspeicher anlegen (research/)" if is_de else "Initialize research archive (research/)"}</span></label>')
        parts.append('        </div>')
        ...
        parts.append(f'          <button type="submit" class="ctl primary" id="btn-submit-import-project" disabled>{"Manuskript importieren & analysieren" if is_de else "Import & Analyze Manuscript"}</button>')
        parts.append('        </div>')
        parts.append('      </form>')
        parts.append('    </div>')

        # Tab Pane 2: Start new project from scratch
        parts.append('    <div class="modal-tab-pane" id="tab-pane-scratch" style="display:none;">')
        parts.append('      <form id="form-project-create" class="modal-body" method="dialog">')
        parts.append('        <div class="project-structure-hint">')
        if is_de:
            parts.append('          <span><strong>Dateien im Projekt:</strong> Erstellt <code>manuscript.md</code> mit der gewählten Kapitelstruktur, <code>lixity.json</code> und optional <code>research/</code>.</span>')
        else:
            parts.append('          <span><strong>Project Files:</strong> Generates <code>manuscript.md</code> with the chosen chapter outline, <code>lixity.json</code>, and optional <code>research/</code>.</span>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <label for="new-proj-title" class="form-label">{L("title")}</label>')
        ...
        parts.append('          </div>')
        parts.append('          <div style="flex:2;">')
        parts.append(f'            <label for="new-proj-path" class="form-label">{"Zielordner (optional)" if is_de else "Target folder (optional)"}</label>')
        ...
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <span class="form-label">{"Erzählstruktur & Vorlage wählen" if is_de else "Select narrative structure & template"}</span>')
        ...
        parts.append(f'          <label class="form-checkbox"><input type="checkbox" id="new-proj-research"/> <span>{"Recherchemodul aktivieren (research/)" if is_de else "Initialize research archive (research/)"}</span></label>')
        parts.append('        </div>')
        parts.append('        <div class="modal-footer">')
        parts.append(f'          <button type="button" class="ctl" data-close-modal>{"Abbrechen" if is_de else "Cancel"}</button>')
        ...
        parts.append('      <div class="form-group">')
        parts.append(f'        <label for="open-proj-path" class="form-label">{"Projektordner oder Manuskriptpfad:" if is_de else "Project folder or manuscript path:"}</label>')
        parts.append(f'        <input class="ctl" id="open-proj-path" required autofocus placeholder="{"/pfad/zu/deinem/projekt oder manuskript.md" if is_de else "/path/to/project or manuscript.md"}" style="width:100%;"/>')
        parts.append(f'        <p class="ctl-note">{"Geben Sie den Pfad zu einem bestehenden Projektordner oder einer manuscript.md an. Lixity öffnet den vorhandenen Arbeitsbereich einschließlich des Recherchespeichers (research/)." if is_de else "Enter the path to an existing project folder or manuscript.md. Lixity opens the existing workspace including its research archive (research/)."}</p>')
        parts.append('      </div>')
        parts.append('      <div class="project-structure-hint" style="margin-top:.6rem;">')
        if is_de:
            parts.append('        <span>Möchten Sie stattdessen eine neue Datei importieren? <button type="button" class="ctl-link" id="link-switch-to-import" style="background:none;border:none;color:var(--accent);text-decoration:underline;cursor:pointer;font-size:inherit;padding:0;">Zum Import-Assistenten</button></span>')
        else:
            parts.append('        <span>Want to import a new file instead? <button type="button" class="ctl-link" id="link-switch-to-import" style="background:none;border:none;color:var(--accent);text-decoration:underline;cursor:pointer;font-size:inherit;padding:0;">Switch to Import Wizard</button></span>')
        parts.append('      </div>')
        parts.append('      <div class="modal-footer">')
        parts.append(f'        <button type="button" class="ctl" data-close-modal>{"Abbrechen" if is_de else "Cancel"}</button>')

...
def render_dashboard(
    ...
    if controls:
        ...
        parts.append(f'<button class="ctl primary" id="r-init-btn">{L("research_init_action")}</button>')
        parts.append('</div>')
        parts.append('</div>')
        ...
        parts.append('      </select>')
        parts.append(f'      <input class="ctl" id="r-claim-tags" placeholder="{L("research_tags")}" style="min-width:160px;"/>')
        parts.append(f'      <select class="ctl" id="r-claim-dossier-select" aria-label="{L("research_dossier")}" style="min-width:180px;"><option value="">{L("research_no_dossier")}</option></select>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append(f'      <textarea class="ctl" id="r-claim-statement" placeholder="{L("research_claim_statement")}" rows="3" style="width:100%;font-family:inherit;"></textarea>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append(f'      <input class="ctl" id="r-claim-time" placeholder="{L("research_claim_time")}" style="flex:1;min-width:150px;"/>')
        parts.append(f'      <input class="ctl" id="r-claim-place" placeholder="{L("research_claim_place")}" style="flex:1;min-width:150px;"/>')
        parts.append(f'      <input class="ctl" id="r-claim-actors" placeholder="{L("research_claim_actors")}" style="flex:1.5;min-width:180px;"/>')
        parts.append('    </div>')
        parts.append('    <div class="row">')
        ...
        parts.append(f'        <option value="contextualizes">{L("research_relation_contextualizes")}</option>')
        parts.append('      </select>')
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append(f'      <input class="ctl" id="r-link-rationale" placeholder="{L("research_link_rationale")}" style="flex:1;"/>')
        ...
        parts.append('    </div>')
        parts.append('    <div class="row" style="margin-bottom:.5rem;">')
        parts.append(f'      <input class="ctl" id="r-decision-plot" placeholder="{L("research_decision_plot")}" style="flex:1;min-width:250px;"/>')
        parts.append('    </div>')
        ...
        parts.append(f'      <button type="button" class="modal-tab-btn active" role="tab" id="tab-btn-import" aria-selected="true" data-tab-target="tab-pane-import">{L("wizard_import_tab")}</button>')
        parts.append(f'      <button type="button" class="modal-tab-btn" role="tab" id="tab-btn-scratch" aria-selected="false" data-tab-target="tab-pane-scratch">{L("wizard_scratch_tab")}</button>')
        parts.append('    </div>')
        ...
        parts.append(f'        <div class="file-dropzone" id="import-dropzone" tabindex="0" role="button" autofocus aria-label="{L("wizard_drop_aria")}">')
        ...
        parts.append(f'            <strong>{L("wizard_drop_main")}</strong>')
        parts.append(f'            <span>{L("wizard_drop_sub")}</span>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="file-preview-card" id="import-preview-box" style="display:none;">')
        parts.append('          <div class="fpc-header">')
        parts.append(f'            <span class="fpc-badge">{L("wizard_recognized")}</span>')
        parts.append('            <span class="fpc-filename" id="import-fpc-filename">manuscript.md</span>')
        parts.append('          </div>')
        parts.append('          <div class="fpc-meta">')
        parts.append(f'            <div class="fpc-stat"><span class="fpc-label">{L("wizard_scope")}</span> <strong id="import-fpc-stats"></strong></div>')
        parts.append(f'            <div class="fpc-stat"><span class="fpc-label">{L("wizard_analysis_language")}</span> <span id="import-fpc-lang"></span></div>')
        parts.append('          </div>')
        parts.append('        </div>')
        ...
        parts.append(f'          <input class="ctl" id="import-proj-title" required placeholder="{L("wizard_import_title_hint")}" style="width:100%;"/>')
        parts.append('        </div>')
        ...
        parts.append(f'            <label for="import-proj-path" class="form-label">{L("wizard_project_folder")}</label>')
        parts.append(f'            <input class="ctl" id="import-proj-path" placeholder="{L("wizard_auto_folder")}" style="width:100%;"/>')
        parts.append(f'            <span class="ctl-note" style="display:block;margin-top:.25rem;font-size:.72rem;">{L("wizard_folder_note")}</span>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <label class="form-checkbox"><input type="checkbox" id="import-proj-research"/> <span>{L("wizard_enable_research")}</span></label>')
        parts.append('        </div>')
        parts.append('        <p class="ctl-status project-form-status" id="import-project-status" role="alert" aria-live="assertive" hidden></p>')
        parts.append('        <div class="modal-footer">')
        parts.append(f'          <button type="button" class="ctl" data-close-modal>{L("modal_cancel")}</button>')
        parts.append(f'          <button type="submit" class="ctl primary" id="btn-submit-import-project" disabled>{L("wizard_import_action")}</button>')
        parts.append('        </div>')
        parts.append('      </form>')
        parts.append('    </div>')
        ...
        parts.append('        <div class="project-structure-hint">')
        parts.append(f'          <span>{L("wizard_project_files")}</span>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <label for="new-proj-title" class="form-label">{L("title")}</label>')
        parts.append(f'          <input class="ctl" id="new-proj-title" required placeholder="{L("wizard_title_example")}" style="width:100%;"/>')
        parts.append('        </div>')
        parts.append('        <div class="form-row">')
        parts.append('          <div style="flex:1;">')
        ...
        parts.append('          </div>')
        parts.append('          <div style="flex:2;">')
        parts.append(f'            <label for="new-proj-path" class="form-label">{L("wizard_target_folder")}</label>')
        parts.append(f'            <input class="ctl" id="new-proj-path" placeholder="{L("wizard_subfolder")}" style="width:100%;"/>')
        parts.append(f'            <span class="ctl-note" style="display:block;margin-top:.25rem;font-size:.72rem;">{L("wizard_folder_note")}</span>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="form-group">')
        parts.append(f'          <span class="form-label">{L("wizard_template_heading")}</span>')
        ...
        parts.append(f'              <div class="tc-title">{L("wizard_minimal")}</div>')
        parts.append(f'              <div class="tc-desc">{L("wizard_minimal_desc")}</div>')
        parts.append('            </label>')
        parts.append('            <label class="template-card">')
        parts.append('              <input type="radio" name="proj_template" value="three_act"/>')
        parts.append(f'              <div class="tc-title">{L("wizard_three_act")}</div>')
        parts.append(f'              <div class="tc-desc">{L("wizard_three_act_desc")}</div>')
        parts.append('            </label>')
        parts.append('            <label class="template-card">')
        parts.append('              <input type="radio" name="proj_template" value="research"/>')
        parts.append(f'              <div class="tc-title">{L("wizard_research_template")}</div>')
        parts.append(f'              <div class="tc-desc">{L("wizard_research_template_desc")}</div>')
        parts.append('            </label>')
        parts.append('          </div>')
        parts.append('        </div>')
        parts.append('        <div class="form-group" style="margin-top:.4rem;">')
        parts.append(f'          <label class="form-checkbox"><input type="checkbox" id="new-proj-research"/> <span>{L("wizard_enable_research")}</span></label>')
        parts.append('        </div>')
        parts.append('        <p class="ctl-status project-form-status" id="new-project-status" role="alert" aria-live="assertive" hidden></p>')
        parts.append('        <div class="modal-footer">')
        parts.append(f'          <button type="button" class="ctl" data-close-modal>{L("modal_cancel")}</button>')
        parts.append(f'          <button type="submit" class="ctl primary" id="btn-submit-create-project">{L("create_project")}</button>')
        parts.append('        </div>')
        parts.append('      </form>')
        parts.append('    </div>')
        parts.append('  </div>')
